		...
		<div class="fivecol noShadow regbox">
			<fieldset>
				<ol>
					...
					<form:select path="project.projectStatus">
					...
					<form:select path="employees" multiple="true" class="et-contact-person-multiple tour2" style="width: 100%">
					...
					<form:select path="propTypes" multiple="true" class="et-property-type-multiple tour2" style="width: 100%">
						<c:forEach items="${propTypes}" var="propType">
							<c:set var="contains" value="" />
							<c:forEach var="item" items="${projectUpdateModel.project.propertyTypes}">
							  <c:if test="${item.propertyTypeID eq propType.propertyTypeID}">
							    <c:set var="contains" value="selected='selected'" />
							  </c:if>
							</c:forEach>
							<option value="${propType.propertyTypeID}" ${contains }>
								${propType.name }
							</option>
						</c:forEach>
					</form:select></li>
